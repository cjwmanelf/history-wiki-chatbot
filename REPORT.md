...
![Streamlit 데모 화면](docs/test.png)

1. **💬 질문하기 탭**:
   - 1홉, 2홉, 3홉, 거절 케이스 원클릭 예시 버튼 제공.
   - 질의 시 **[답변 요약]**, **[탐색 및 추론 경로]**(화살표 방향 표기), **[근거 삼중항 표]**(신뢰도/출처 표기), **[출처 문서 원문 펼쳐보기]**(expander) 일체 노출.
2. **📊 평가 결과 탭**:
   - `output/eval.json`과 연동되어 홉별 GraphRAG vs Basic RAG 대조표, 거절 가드레일 수치, 실패 계층 분류 카드 실시간 렌더링.
3. **⚙️ 설정 탭**:
   - OpenAI API 키 입력 시 패스워드 마스킹 및 `sk-********` 안전 노출.
   - 키 없이도 100% 정상 작동하며, 런타임 주입/안전 파일 저장/삭제 및 즉시 연결 테스트 기능 제공.
