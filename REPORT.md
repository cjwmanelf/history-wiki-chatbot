...
- **종합 판정**: 전체 14개 문항(답변 가능 11건 + 거절 3건) 전수 평가에서 최종 실패 사례는 0건이다.

#### 4.4.1 최종 0건을 그대로 믿으면 안 되는 이유 (한계 명시)

최종 수치가 전부 만점이라는 사실만 보고하면 정직하지 않다. 두 가지를 분명히 해 둔다.

**(1) 색인 실패 0건은 부분적으로 구성상 당연하다.**
골든셋의 `expected_path` 를 저장 직전에 그래프와 대조 검증했기 때문에,
*그래프에 없는 사실을 묻는 `answerable` 문항은 애초에 골든셋에 들어갈 수 없다.*
따라서 이 평가가 실제로 측정하는 것은 **색인 커버리지가 아니라
"이미 색인된 사실을 탐색과 생성이 온전히 회수하는가"** 다.
색인층을 제대로 측정하려면 그래프를 보지 않고 **원문만 읽어 독립적으로 만든 별도 골든셋**이 필요하다.
이건 이번 범위에서 하지 못했고, 6.2에 보완 과제로 남긴다.

**(2) 3홉은 문항이 1건뿐이다.** 3홉 100%는 표본 1개의 결과이므로 일반화할 수 없다.

#### 4.4.2 실제로 관측된 실패와 수정 이력 (1차 자료)

"실패 사례를 직접 읽어 어느 층에서 깨졌는지 구분"한 결과는 **최종 스냅샷이 아니라 이 과정에 있다.**
전 구간을 5회 측정했고, 매 회차의 실패를 층으로 분류해 원인을 읽고 고쳤다.
전체 기록은 [`output/eval_progression.json`](output/eval_progression.json) 에 있다.

| 회차 | 구성 | 실패 층 (색인/탐색/생성) | 관측된 실패와 직접 읽은 원인 | 적용한 수정 |
| :---: | :--- | :---: | :--- | :--- |
| 1 | 픽스처 골든셋 + 픽스처 그래프 | 0 / 1 / 0 | BasicRAG 가 **전 구간 0.000**. 대조군이 망가진 채로 한 비교라 무의미 | 평가 시점에 `data/docs` 가 비어 있던 것이 원인. 코퍼스 완성 + BasicRAG 한국어 토크나이저를 문자 n-gram(`char_wb`)으로 교체 |
| 2 | 픽스처 골든셋 + **실물** 그래프 | **8** / 0 / 1 | 색인 실패 8건. GraphRAG(0.400)가 BasicRAG(0.450)보다 **낮았다** | 골든셋과 그래프의 출처가 달랐다. 픽스처가 기대한 삼중항(안중근·우덕순·대한국민회)은 실물 그래프에 없다. 시스템 결함이 아니라 **평가 구성 오류**였다 → 실물 골든셋 14문항으로 교체 |
| 3 | 실물 골든셋 + 실물 그래프 (규칙) | 0 / **4** / **1** | 2홉에서 대조군을 못 이김(0.6 vs 0.6) | **탐색**: 2홉 질문인데 시드 자신의 `FOUNDED` 엣지로 1홉 답변을 냈다. "김좌진이 참여한 전투에 **함께 참전한 인물이** 세운 조직은?" → 김좌진 자신의 조직을 답함. Q07 은 정답 목록에 우연히 겹쳐 `answer_acc 1.0` 을 받았고 **`path_recall` 0.8 이 오답 추론을 잡아냈다**. **생성**: Q09 는 근거가 컨텍스트에 있는데도 거절했다(답 타입이 Organization 이 아니라 Person 인 교집합형 질문) → 경유 신호가 있으면 `Other != seed` 경로만 채택, 답 타입 판정 일반화 |
| 4 | 실물 + **LLM 증강** (합성 수정 전) | 0 / 0 / **1** | LLM 을 켜면 **오히려 점수가 떨어졌다** (규칙 1.000 vs LLM 0.955) | **생성**: LLM 이 탄 경로와 다른 사실을 서술. 기계 기록은 `청산리 전투` 인데 산문은 `봉오동 전투` 라고 했다. 원인은 LLM 에게 후보 경로 5개와 삼중항 30개를 통째로 주고 3블록 전체를 쓰게 한 것 → **LLM 역할을 "확정된 요약의 문장 다듬기"로 축소**, 경로·출처 블록은 코드가 소유, 개체 이탈 검증 후 폴백 |
| 5 | 최종 (두 모드) | 0 / 0 / 0 | — | — |

이 표가 보여주는 것은 **층 분류가 실제로 고칠 곳을 지목했다**는 점이다.
2회차의 색인 실패는 추출기가 아니라 평가 구성을 고치게 했고,
3회차의 탐색 실패는 경로 선택 규칙을, 4회차의 생성 실패는 합성 경계를 고치게 했다.

특히 3회차에서 **`answer_acc` 는 만점인데 `path_recall` 이 0.8 로 떨어진 문항(Q07)** 이
"정답은 맞았지만 추론은 틀렸다"를 드러냈다. 정답률만 봤다면 놓쳤을 결함이다.
경로 재현율을 정답률과 **분리해서** 측정한 것이 이 프로젝트에서 가장 값어치를 한 결정이었다.

#### 4.4.3 두 실행 모드의 측정값

API 키 없이도 전 파이프라인이 동작하므로 **두 모드를 각각 측정**했다.
채점자가 키 없이 재현하는 경로가 규칙 전용 모드다.

| 홉 수 | 규칙 전용 (키 없음) GraphRAG | 규칙 전용 BasicRAG | LLM 증강 GraphRAG | LLM 증강 BasicRAG |
| :---: | :---: | :---: | :---: | :---: |
| 1홉 (n=5) | 1.000 | 0.500 | 1.000 | 0.600 |
| 2홉 (n=5) | 1.000 | 0.600 | 1.000 | 0.400 |
| 3홉 (n=1) | 1.000 | 0.500 | 1.000 | 0.500 |
| **전체** | **1.000** | 0.545 | **1.000** | 0.500 |
| 실패 층 | 0 / 0 / 0 | — | 0 / 0 / 0 | — |

산출물: [`output/eval_rule_only.json`](output/eval_rule_only.json) · [`output/eval_llm_mode.json`](output/eval_llm_mode.json)

**LLM 증강은 정답률을 올리지 못했다.** 규칙 기반 템플릿이 이미 만점이라 올릴 여지가 없었고,
오히려 4회차처럼 **근거 이탈을 새로 만들어 점수를 깎았다.**
그래서 최종 설계에서 LLM 은 사실을 고르는 자리에서 빼고 **문장을 다듬는 자리로만** 남겼다.
개체가 어긋나면 결정론적 문장으로 되돌리고 그 사실을 `runs.jsonl` 의
`llm_fallback_reason` 에 남긴다. 이 도메인에서 얻은 결론은
**"삼중항이 이미 답을 확정한 뒤에는 LLM 이 더할 것이 문장뿐"** 이라는 것이다.
...
0. **색인층을 실제로 측정하는 독립 골든셋 (가장 아쉬운 부분)**:
   - 현재 골든셋은 `expected_path` 가 그래프에 존재하는지 저장 직전에 대조 검증한다.
     덕분에 평가가 깨끗하게 돌아가지만, 그 대가로 **그래프에 없는 사실을 묻는 문항이 구조적으로 배제된다.**
     그래서 최종 색인 실패 0건은 "추출이 완벽하다"가 아니라 "그렇게 물어보지 않았다"에 가깝다(4.4.1 참조).
   - 보완하려면 **그래프를 보지 않고 원문 문서만 읽어 독립적으로 만든 골든셋**이 필요하다.
     그걸로 재면 4.4.2의 알려진 추출 한계(다중 인물 문장의 주어 오선택, 역접·부정 절 무시)가
     색인 실패로 정직하게 잡힐 것이다. 이번에는 코퍼스·그래프·골든셋을 같은 파이프라인에서
     만들었기 때문에 그 측정을 하지 못했다. 다음에 한다면 **골든셋을 먼저 만들고 그래프를 나중에 짓는
     순서**로 뒤집겠다.
